...
class ResNetASPP(torch.nn.Module):
    def __init__(self, *args, **kwargs):
        ...
        # create the head:
        self.sic_decoder = nn.Sequential(ASPP(in_channels=1024, atrous_rates = [12, 24, 36], out_channels = 32),
                                        nn.Conv2d(32, 32, 3, padding=1, bias=False),
                                        nn.BatchNorm2d(32),
                                        nn.ReLU(),
                                        nn.Conv2d(32, 1, 1))

        self.sod_decoder = nn.Sequential(ASPP(in_channels=1024, atrous_rates = [12, 24, 36], out_channels = 32),
                                        nn.Conv2d(32, 32, 3, padding=1, bias=False),
                                        nn.BatchNorm2d(32),
                                        nn.ReLU(),
                                        nn.Conv2d(32, kwargs['n_classes']['SOD'], 1))

        self.floe_decoder = nn.Sequential(ASPP(in_channels=1024, atrous_rates = [12, 24, 36], out_channels = 32),
                                        nn.Conv2d(32, 32, 3, padding=1, bias=False),
                                        nn.BatchNorm2d(32),
                                        nn.ReLU(),
