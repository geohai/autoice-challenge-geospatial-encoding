...
from visualizations import plot_scene_confusion_matrix, plot_model_predictions, plot_test_confusion_matrix, plot_class_frequencies
...
def main(config:ConfigParser):
    ...
    train_options = {
        ...
        'train_variables': SCENE_VARIABLES[0:16],  # Contains the relevant variables in the scenes.
    ...
    loss_functions = { 'SIC': mse, 
                  'SOD': torch.nn.CrossEntropyLoss(ignore_index=train_options['class_fill_values']['SOD']), 
                  'FLOE': torch.nn.CrossEntropyLoss(ignore_index=train_options['class_fill_values']['FLOE']) }
    ...
    for epoch in tqdm(iterable=range(train_options['epochs']), position=0):
        ...
        for inf_x, inf_y, masks, name in tqdm(iterable=dataloader_test, total=len(train_options['test_list']), colour='green', position=0):
            scene_name = name[:18]  # Removes the _prep.nc from the name.
            torch.cuda.empty_cache()

            mask = inf_y['SIC'] != 255
            mask = mask.to(device)
            inf_y['SIC'] = inf_y['SIC'].to(device)
            inf_y['SIC'] = torch.where(mask, inf_y['SIC'], torch.tensor(float('nan')).to(device))
            ...
            with torch.no_grad(), torch.cuda.amp.autocast():
                ...
                for chart in train_options['charts']:
                    if chart == 'SIC':
                        mask = output['SIC'] != 255
                        output['SIC'] = torch.where(mask, output['SIC'], torch.tensor(float('nan')).to(device))
                        loss_batch_test += loss_functions[chart](train_options,output[chart], inf_y[chart].to(device),1)
                    else:
                        loss_batch_test += loss_functions[chart](input=output[chart], target=inf_y[chart].to(device))
            ...
            for chart in train_options['charts']:
                if chart == 'SIC':
                    output[chart] = output[chart].squeeze(dim=0).cpu().numpy()
                    #print(f"Shape of the SIC output '{chart}': {output[chart].shape}")
                else:
                    output[chart] = torch.argmax(output[chart], dim=1).squeeze().cpu().numpy()
                    #print(f"Shape of the SOD and FLOE output '{chart}': {output[chart].shape}")
                outputs_flat[chart] = np.append(outputs_flat[chart], output[chart][~masks[chart]])
                inf_y[chart]= inf_y[chart].cpu()
                inf_ys_flat[chart] = np.append(inf_ys_flat[chart], np.squeeze(inf_y[chart], 0)[~masks[chart]].numpy())
            ...
            for idx, chart in enumerate(train_options['charts']):
                ...
                output[chart] = torch.from_numpy(output[chart])
                chart_mask = masks[chart].numpy()
                output[chart][chart_mask] = np.nan
        ...
        plot_test_confusion_matrix(inf_ys_flat,
                               outputs_flat,
                               train_options,
                               model_name="20_all",
                               normalization='all')
        ...
        plot_class_frequencies(inf_ys_flat, train_options, model_name="20_true")
        plot_class_frequencies(outputs_flat, train_options, model_name="20_pred")
