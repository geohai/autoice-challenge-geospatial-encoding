...
class AI4ArcticChallengeDataset(Dataset):
    ...
    def random_crop(self, scene):
        ...
        inp_variables = list(self.options['full_variables'][0:6]) + lat_variables + list(self.options['full_variables'][8:9])
        #print(self.options['full_variables'])
        #print(inp_variables)
        ...
        if np.sum(scene['SIC'].values[row_rand: row_rand + self.options['patch_size'], 
            ...
            if len(self.options['amsrenv_variables']) > 0:
                ...
                patch[len(inp_variables):, :, :] = torch.nn.functional.interpolate(
        ...
        # print('This is the shape returned by RC',patch.shape)
        # print(patch)
        return patch
    ...
    def lonlat_normalize(self, scene, extent=(-180, 180, -90, 90), do_global=True):
        if do_global:
            scene['sar_grid2d_longitude'] /= 180.0
            scene['sar_grid2d_latitude'] /= 90.0
        else:
            scene['sar_grid2d_longitude'] = ((scene['sar_grid2d_longitude'] - extent[0]) / (extent[1] - extent[0])) * 2 - 1
            scene['sar_grid2d_latitude'] = ((scene['sar_grid2d_latitude'] - extent[2]) / (extent[3] - extent[2])) * 2 - 1
        return scene
    ...
    def __getitem__(self, idx):
        ...
        patches = np.zeros((self.options['batch_size'], self.patch_c,
                            self.options['patch_size'], self.options['patch_size']))
        sample_n = 0
        #print('This is the shape of patches before while',patches.shape)
        ...
        while sample_n < self.options['batch_size']:
            ...
            #print('This is the shape of scene after resize, normalize, and encode',scene)            
            # - Extract patches
            scene_patch = self.random_crop(scene)
            #print('This is shape of the patch returned by random crop',scene_patch.shape)
            if scene_patch is not None:
                # -- Stack the scene patches in patches
                patches[sample_n, :, :, :] = scene_patch
                sample_n += 1 # Update the index.

        #print('This is the shape of patches',patches.shape)
...
class AI4ArcticChallengeTestDataset(Dataset):
    ...
    def prep_scene(self, scene):
        ...
        #print(inp_variables)
        if len(self.options['amsrenv_variables']) > 0:
            x = torch.cat((torch.from_numpy(scene[np.array(inp_variables)[~np.isin(inp_variables, 
