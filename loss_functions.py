...
def bce(options: dict, output, target, bins_w):
    ...
    true_pixels = torch.sum(
        target != options['class_fill_values']['SIC'])

    # Convert target to [0, 1]
    target_small = torch.clamp(target.type(
        torch.float) / (options['n_classes']['SIC'] - 1), max=1.)
    ...
    bce = torch.sum(torch.nn.functional.binary_cross_entropy_with_logits(
        input=output, target=target_small, reduction='none')) / true_pixels
...
def emd_2(options: dict, output, target, bins_w):
    ...
    # Get number of true pixels
    true_pixels = torch.sum(
        target != options['class_fill_values'][options['chart']])
...
def mse(options: dict, output, target, bins_w=1):
    """
    ...
    # print('before squeeze output_n shape is', output.shape)
    # print('before squeeze target_n shape is', target.shape)
    # output = output.permute(0, 2, 3, 1)

    target_n = target.squeeze()
    output_n = output.squeeze()

    mask = torch.isfinite(output_n) & torch.isfinite(target_n)

    output_n = output_n[mask]
    target_n = target_n[mask]

    # print('after squeeze output_n shape is', output_n.shape)
    # print('after squeeze target_n shape is', target_n.shape)
    # Get number of true pixels
    true_pixels = torch.sum(
        target_n != options['class_fill_values']['SIC'])
    ...
    sum_stage = torch.sum(torch.nn.functional.mse_loss(
        input=output_n, target=target_n.type(torch.float), reduction='none'))

    mse = sum_stage / true_pixels
