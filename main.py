...
from loss_functions import bce,mse
...
def main(config:ConfigParser):
    ...
    min_loss_epoch_val = 999  # Validation loss.
    ...
    for epoch in tqdm(iterable=range(train_options['epochs']), position=0):
        ...
        for i, (batch_x, batch_y) in enumerate(tqdm(iterable=dataloader, total=train_options['epoch_len'], colour='red', position=0)):
            ...
            mask = batch_y['SIC'] != 255
            mask = mask.to(device)
            batch_y['SIC'] = batch_y['SIC'].to(device)
            batch_y['SIC'] = torch.where(mask, batch_y['SIC'], torch.tensor(float('nan')).to(device))
            ...
            with torch.cuda.amp.autocast():
                ...
                for chart in train_options['charts']:
                    if chart == 'SIC':
                        ...
                        loss_batch += loss_functions[chart](train_options,output[chart], batch_y[chart].to(device),1)
                    else:
                        loss_batch += loss_functions[chart](input=output[chart], target=batch_y[chart].to(device))
                ...
                for chart in train_options['charts']:
                    if chart == 'SIC':
                        mask = output['SIC'] != 255
                        output['SIC'] = torch.where(mask, output['SIC'], torch.tensor(float('nan')).to(device))
                        loss_batch_c[chart] = loss_functions[chart](train_options,output[chart], batch_y[chart].to(device),1)
                    else:
                        loss_batch_c[chart] = loss_functions[chart](input=output[chart], target=batch_y[chart].to(device))
            ...
            with torch.no_grad(), torch.cuda.amp.autocast():
                ...
                for chart in train_options['charts']:
                    if chart == 'SIC':
                        mask = output['SIC'] != 255
                        output['SIC'] = torch.where(mask, output['SIC'], torch.tensor(float('nan')).to(device))
                        loss_batch_val += loss_functions[chart](train_options,output[chart], inf_y[chart].to(device),1)
                    else:
                        loss_batch_val += loss_functions[chart](input=output[chart], target=inf_y[chart].to(device))
        ...
        if combined_score > best_combined_score:
            ...
            torch.save(obj={'model_state_dict': net.state_dict(),
                            ...
                            'epoch': epoch},
                            f=os.path.join(dir_out, 'best_score_model.ckpt'))
